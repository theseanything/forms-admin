...
describe "pages/conditions/delete.html.erb" do
  ...
  let(:condition) { create(:condition, form:, routing_page_id: pages.first.id, check_page_id: pages.first.id, answer_value: "Wales", goto_page_id: pages.last.id) }
  ...
  context "when the condition has an exit page" do
    let(:condition) { create(:condition, :with_exit_page, form:, routing_page_id: pages.first.id, check_page_id: pages.first.id) }
